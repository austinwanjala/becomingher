...
export default function ProgrammesDashboardPage() {
  ...
  const serviceId = typeof params?.id === 'string' ? params.id : 'srv-guided-01';
  ...
  const handleQuestionnaireUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !user || !entitlement) return;
    ...
    try {
      ...
      const fileName = `filled-${user.id}-${Date.now()}.${fileExt}`;
      ...
      const { error: updateErr } = await supabase
        .from('entitlements')
        .update({ questionnaire_url: fileUrl })
        .eq('id', entitlement.id);

      if (updateErr) throw updateErr;

      setEntitlement((prev: any) => ({ ...prev, questionnaire_url: fileUrl }));
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ========================================================================= */}
      {/* REQUIRED CUSTOMIZED QUESTIONNAIRE                                         */}
      {/* ========================================================================= */}
      {service?.questionnaire_template_url && entitlement && (
        <div className="bg-rose-50/50 p-6 sm:p-8 rounded-3xl border border-rose-200/60 shadow-sm space-y-5">
          <div className="space-y-1">
            <h3 className="font-serif text-xl font-semibold text-rose-950 flex items-center gap-2">
              <FileCheck className="w-5 h-5 text-rose-700" />
              Required Questionnaire
            </h3>
            <p className="text-xs text-rose-800/80">
              For your Customized Digital Coaching, please download this questionnaire, fill it out thoroughly, and upload the completed version below before your session.
            </p>
          </div>

          <div className="flex flex-col sm:flex-row items-center gap-4">
            <a
              href={service.questionnaire_template_url}
              target="_blank"
              rel="noreferrer"
              className="w-full sm:w-auto px-5 py-3 rounded-xl bg-white border border-rose-200 text-rose-900 text-xs font-semibold hover:bg-rose-100 transition shadow-sm flex items-center justify-center gap-2"
            >
              <Download className="w-4 h-4" />
            ...
            <div className="flex-1 w-full relative">
              {entitlement.questionnaire_url ? (
                <div className="flex items-center justify-between bg-white border border-green-200 p-3 rounded-xl shadow-sm">
                  <div className="flex items-center gap-2 text-green-700 text-xs font-semibold">
                    ...
                    <span>Questionnaire Uploaded Successfully</span>
                  </div>
                  <a
                    href={entitlement.questionnaire_url}
                    target="_blank"
                    rel="noreferrer"
                ...
                <label className={`cursor-pointer w-full flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-amber-50 text-xs font-semibold shadow-sm transition ${isUploading ? 'bg-stone-500' : 'bg-stone-900 hover:bg-rose-950'}`}>
                  {isUploading ? (
                    <span className="animate-pulse">Uploading...</span>
                  ) : (
                    <>
                      <ExternalLink className="w-4 h-4" />
                      <span>Upload Filled Questionnaire</span>
                    </>
                  )}
                  <input
                    type="file"
              ...
              {uploadError && (
                <p className="text-[10px] text-rose-600 mt-1.5 absolute -bottom-5 left-0">{uploadError}</p>
              )}
            </div>
          </div>
        </div>
